...
test.each([Color.black, Color.white])("Should retrieve pieces from team [%s]", (color: Color) => {
  ...
  board.place(p1, pos(1, 1));
  board.place(p2, pos(1, 2));
  board.place(p3, pos(1, 3));
  board.place(new Knight(colorInvert(color)), pos(5, 5));
  board.place(new Knight(colorInvert(color)), pos(5, 6));
  board.place(new Knight(colorInvert(color)), pos(5, 7));
...
test("Should retrieve king throws an error if it has more than 1 king (same team) on board", () => {
  const board = Board.empty();
  board.place(new King(Color.black), pos(1, 1));
  board.place(new King(Color.black), pos(1, 2));
  expect(() => board.getKing(Color.black)).toThrowError();
});
...
test("Should clone board and pieces", () => {
  ...
  board.place(ref1, pos(1, 2));
  board.place(ref2, pos(1, 3));
  board.place(ref3, pos(1, 4));
  board.place(ref4, pos(1, 5));
